...
def creator_events():
    if session.get('useronline')!=None:
        ...
        return render_template('creator/creator_events.html', events=events, category=category, creator=creator, state=state, lga=lga, id=id, user=user, current_page='events')
    else:
...
def creator_comms():
    if session.get('useronline')!=None:
        ...
        return render_template('creator/creator_comm.html', comms=comms, category=category, creator=creator, user=user, current_page='communities')
    else:
        flash('You must be logged in to view this page', category='errormsg')
        return redirect(url_for('login'))
...
def state_lgas():
    ...
    if request.method == 'POST':
        ...
        eventform.lga_id.choices = [(l.id, l.name) for l in lgas]
    return 'Successful'
...
@app.route('/creator/edit-event/<id>/', methods=['POST', 'GET'])
...
@app.route('/creator/delete-event/<id>/')
def delete_event(id):
    event= Event.query.get_or_404(id)
